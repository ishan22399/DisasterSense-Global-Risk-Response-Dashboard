...
function InteractiveMap({ disasters, onDisasterClick, getSeverityColor, selectedDisaster, isVisible, mapLayer }: {
  ...
  const mapLayers = [
    ...
    { id: "dark", name: "Dark Mode", url: "https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}.png" }
  ]
  ...
  const loadMap = async () => {
    try {
      ...
      try {
        // Remove any problematic template variables that aren't standard Leaflet variables
        tileUrl = tileUrl
          .replace(/\{r\}/g, '') // Remove retina variables
          .replace(/\{time\}/g, '') // Remove time variables
          .replace(/\{format\}/g, 'png') // Replace format with png
          .replace(/\{api_key\}/g, '') // Remove api_key variables
          .replace(/\{\w+\}/g, (match) => {
            // Keep standard Leaflet variables: {s}, {z}, {x}, {y}
            if (['s', 'z', 'x', 'y'].includes(match.slice(1, -1))) {
              return match
            }
            return '' // Remove non-standard variables
          })
        
        // Ensure the URL has required Leaflet variables
        if (!tileUrl.includes('{z}') || !tileUrl.includes('{x}') || !tileUrl.includes('{y}')) {
          tileUrl = "https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
        }
      } catch (error) {
        console.warn('Error processing tile URL:', error)
        tileUrl = "https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
      }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6">
        ...
        <CardHeader className="pb-3 sm:pb-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between space-y-3 sm:space-y-0">
            <div className="flex items-center space-x-2">
              ...
              <div>
                <CardTitle className="text-lg sm:text-xl">Live Disaster Map</CardTitle>
                <CardDescription className="text-sm">Interactive map showing real-time disaster locations</CardDescription>
              </div>
            </div>
            <Button 
              variant="outline" 
            ...
            <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-2">
              ...
              <div className="grid grid-cols-2 sm:flex sm:space-x-1 gap-1 sm:gap-0">
                <Button
                  ...
                  className={`text-xs sm:text-sm ${mapLayer === "street" ? "bg-blue-600 text-white" : ""}`}
                >
                  <span className="hidden sm:inline">🗺️ Street</span>
                ...
                <Button
                  ...
                  className={`text-xs sm:text-sm ${mapLayer === "satellite" ? "bg-blue-600 text-white" : ""}`}
                >
                ...
                <Button
                  ...
                  className={`text-xs sm:text-sm ${mapLayer === "terrain" ? "bg-blue-600 text-white" : ""}`}
                >
                  <span className="hidden sm:inline">🏔️ Terrain</span>
                  <span className="sm:hidden">🏔️</span>
                </Button>
                <Button
                  ...
                  className={`text-xs sm:text-sm ${mapLayer === "dark" ? "bg-blue-600 text-white" : ""}`}
                >
      ...
      <Card className="lg:col-span-1">
        <CardHeader className="pb-3 sm:pb-6">
          <CardTitle className="text-lg sm:text-xl">Active Incidents</CardTitle>
          <CardDescription>Click markers for details</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3 sm:space-y-4">
          <div className="max-h-64 sm:max-h-80 lg:max-h-96 overflow-y-auto space-y-3">
              ...
              return (
                ...
                >
                  <div className="flex items-start space-x-2 sm:space-x-3">
                    <div className={`p-1.5 sm:p-2 rounded-full ${getSeverityColor(disaster.severity)} flex-shrink-0`}>
                      <Icon className="h-3 w-3 sm:h-4 sm:w-4 text-white" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-sm truncate">{disaster.title}</p>
                      <p className="text-xs text-gray-500 mt-1">
                        <span className="hidden sm:inline">Magnitude: {disaster.magnitude} | </span>
                      ...
                      <div className="flex items-center justify-between mt-2">
                        ...
                        <span className="text-xs text-gray-400 hidden sm:inline">
                          {new Date(disaster.time).toLocaleDateString()}
                        </span>
                      </div>
                    </div>
                  </div>
